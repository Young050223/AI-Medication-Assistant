...
### 新增功能
- **删除范围弹窗** — 删除计划时弹出「仅删除当天 / 删除未来全部」范围选择对话框（复用编辑范围的交互模式）
  - 「仅删除当天」— 写入 `dateOverride.isDeleted = true` 实现软删除，保留历史记录
  - 「删除未来全部」— 将 `endDate` 截断至选定日期前一天，丢弃未来 override 数据
  - 若选定日期 ≤ 计划开始日期，直接物理删除整个计划
- **`ScheduleOverride.isDeleted` 字段** — `useMedicationSchedule` 新增 `isDeleted` 属性，日历圆点和日程列表均过滤已软删除的日期

### 优化
- **日历默认定位到今天** — 进入页面时日历定位到当天而非用药周期锚点日期，提升用户体验
- **日期 key 归一化** — `medicationDates` 计算使用 `normalizeDateKey` 统一日期格式，避免 ISO 时间戳与纯日期 key 不匹配
- **新增 i18n 翻译键** — 删除范围弹窗相关文案（简/繁中文 + 英文）
